...
class CombinationParametersDoc:
    ...
    def signature_end(self, is_generic_function):
        parameters_names = self.method_entry["parameters"].keys()
        if parameters_names:
            if is_generic_function:
                segment = f", method={self.method_key}"
            else:
                segment = ""
    ...
    def run(self, verbose=False):
        ...
        with open(self.output_path, "w") as f:
            ...
            f.write('.. function:: ctfr.ctfr(<other parameters>')
            f.write(self.signature_end(True) + "\n" + "   :noindex:" + "\n\n")
            f.write('.. function:: ctfr.ctfr_from_specs(<other parameters>')
            f.write(self.signature_end(True) + "\n" + "   :noindex:" + "\n\n")
            f.write(f'.. function:: ctfr.methods.{self.method_key}(<other parameters>')
            f.write(self.signature_end(False) + "\n" + "   :noindex:" + "\n\n")
            f.write(f'.. function:: ctfr.methods.{self.method_key}_from_specs(<other parameters>')
            f.write(self.signature_end(False) + "\n" + "   :noindex:" + "\n\n")
            f.write("See :func:`ctfr.ctfr` and :func:`ctfr.ctfr_from_specs` for more details on the general parameters. The parameters specific to this method (passed as keyword arguments) are described below." + "\n\n")
